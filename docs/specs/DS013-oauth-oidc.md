...
The Google auth wildcard retains guest transport admission; it does not gain an overlapping public-route override. Local redirects are fixed same-origin relative paths with the public Router prefix; Google authorization uses same-origin initiation fetch followed by explicit navigation to the verified HTTPS URL. The OIDC Google button exists only in the wizard rendered by the module script, so it cannot submit the JSON initiation endpoint as a native navigation before the script loads. OIDC cookie paths stay unchanged. Callback/redirect responses use no-store, no-referrer, no third-party assets and no code/token logging. Google resume forms and ordinary interaction pages use `Referrer-Policy: same-origin` so native form POSTs retain the exact Origin required for CSRF checks. OIDC resume form CSP permits the retained, validated client callback origin through the authorization redirect chain; it cannot select an arbitrary destination. Framing remains denied.
...
Claim the installation first (for example with **Administrator sign-in** after configuring `USERPERSISTO_ADMIN_PASSWORD`, see DS012), retain the generated settings key, configure the issuer on the UserPersisto agent, and restart it. From an administrator session, open **Settings → Administration → Applications** and verify the displayed issuer/discovery URL. Register exact application callbacks and save any generated confidential secret in the application server's secret store.
