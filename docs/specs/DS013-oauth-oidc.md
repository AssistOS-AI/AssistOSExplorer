...
The login prompt renders the shared sign-in wizard defined in DS012: a server shell (application name, an abort form with the interaction CSRF token, and the wizard configuration as an inert JSON script) plus the same-origin `auth/oidc-main.js` module. Read-only and preparatory steps are form-encoded POSTs to `/interaction/{uid}/{action}` carrying the CSRF token and the browser's Origin: `attempt`, `attempt-cancel`, `discover`, `email-start`, `passkey-options` and `google`. Credential completions are native form POSTs: `email-verify`, `totp`, `passkey-verify` (the email and challenge stay server-side from `passkey-options`) and `admin-login`. Success ends in the engine's `interactionFinished` redirect with the method in `amr` (`emailCode`, `totp`, `passkey`, `pwd`, or `federated` for Google). A failure re-renders the wizard with status 400, a visible error, and only the attempted email, never codes or passwords. A disabled method returns `access_denied`. A GET on an interaction whose result was already committed redirects to the engine's resume URL, so a lost response after success is safe. Codes and browser binding follow DS012: the `up_browser` cookie path is the issuer path minus `oidc`, and OIDC attempts use the shared rate-source bucket plus per-parent and per-address limits.
...
An authorization request may include the optional UI extension `screen_hint=signup`. If the engine requires a login interaction and registration is available, the wizard starts in Register mode (**Create your account**). Absent or unrecognized hints preserve the Login-first page, except that an unclaimed installation also starts in Register mode. No hint changes the registration role, enables a disabled method, forces a new login for an existing provider session, or bypasses consent. The hint is retained by the provider's authorization engine and is not taken from arbitrary interaction-page query parameters.
...
Claim the installation first (for example with **Administrator sign-in** and the new-installation default `admin`, or an operator-configured `USERPERSISTO_ADMIN_PASSWORD`, see DS012), retain the generated settings key, configure the issuer on the UserPersisto agent, and restart it. From an administrator session, open **Settings → Administration → Applications** and verify the displayed issuer/discovery URL. Register exact application callbacks and save any generated confidential secret in the application server's secret store.
