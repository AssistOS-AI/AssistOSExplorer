...
The presence of `PROD` forces the Google-only sign-in contract in DS012 for OIDC as well as Explorer SSO. Any defined value, including an empty string or `false`, enables it. The wizard exposes only Google sign-in or a Google-unavailable message; email/password fields, local submit and registration switches, saved local attempts and local-error recovery screens cannot provide another route. Email-code, passkey, TOTP and administrator-password actions, including retained non-Google completions and replay, are refused under the effective server policy. Google registration still follows self-registration and first-owner rules, and application scope consent remains explicit.
...
The login prompt renders the shared sign-in wizard defined in DS012: a server shell (application name, an abort form with the interaction CSRF token, and the wizard configuration as an inert JSON script) plus the same-origin `auth/oidc-main.js` module. The opening form orders Google sign-in when available, **Email**, the optional **Admin password** field when available, and **Sign in**; a filled password submits `admin-login`, while a blank password continues the ordinary email flow. Read-only and preparatory steps are form-encoded POSTs to `/interaction/{uid}/{action}` carrying the CSRF token and the browser's Origin: `attempt`, `attempt-cancel`, `discover`, `email-start`, `passkey-options` and `google`. Credential completions are native form POSTs: `email-verify`, `totp`, `passkey-verify` (the email and challenge stay server-side from `passkey-options`) and `admin-login`. The administrator password is cleared from the visible form immediately on submission. Before setup, the optional email travels as `contactEmail`; after setup, the email never selects an administrator. Success ends in the engine's `interactionFinished` redirect with the method in `amr` (`emailCode`, `totp`, `passkey`, `pwd`, or `federated` for Google). A failure re-renders the wizard with status 400, a visible error, and only the attempted email, never codes or passwords. An `admin-login` failure returns to the same opening form and retains any submitted contact email. A disabled method returns `access_denied`. A GET on an interaction whose result was already committed redirects to the engine's resume URL, so a lost response after success is safe. Codes and browser binding follow DS012: the `up_browser` cookie path is the issuer path minus `oidc`, and OIDC attempts use the shared rate-source bucket plus per-parent and per-address limits.

Only enabled methods are offered and accepted. Email-code sign-in never creates an unknown account; registration sends a code to an unused address and creates the account only on the correct code, through DS012's setup decision: the first completed sign-in of an unclaimed installation becomes its administrator, and later registrations require self-registration and receive `selfRegistered` with dashboard-only access. Disabling registration rejects it at completion too, including an attempt started before the policy changed; a late same-email collision reports `account_exists` so the wizard can switch to sign-in. TOTP and passkey sign-in require an existing enrollment from the standalone My Account dashboard. The administrator-password action is available only when the default or configured administrator password is usable under DS012 and never approves application scopes. There are no ordinary password `login` or `register` actions. Recovery, step-up authentication and general email replacement remain outside this scope.

An authorization request may include the optional UI extension `screen_hint=signup`. If the engine requires a login interaction and registration is available, the wizard's email flow starts in Register mode (**Create your account**, with **Continue** when the administrator password is also available). On an unclaimed installation with the administrator password available, the unified form instead retains its **Sign in** heading and submit button and hides the Login/Register switch; its blank-password email flow still registers the first account. Absent or unrecognized hints preserve the Login-first page, except that an unclaimed installation also starts its email flow in Register mode. No hint changes the registration role, enables a disabled method, forces a new login for an existing provider session, or bypasses consent. The hint is retained by the provider's authorization engine and is not taken from arbitrary interaction-page query parameters.
...
| Resume action POSTs | Authenticate/challenge/send-link-code/verify-link-code/confirm-link/send-email-proof/verify-email-proof/cancel require exact Origin, transaction proof, purpose-bound CSRF and original parent; bodies do not choose a target or destination. `authenticate` accepts only passkey, TOTP or the administrator password for the designated administrator; `confirm-link` may carry `method=googleAuthoritative` for the narrow shortcut in DS012. |
...
Claim the installation first through Google when `PROD` is present. With `PROD` absent, another option is to enter the new-installation default `admin` or an operator-configured `USERPERSISTO_ADMIN_PASSWORD` in the opening form's **Admin password** field and select **Sign in** (DS012); the optional **Email** records only an unverified contact address. Retain the generated settings key, configure the issuer on the UserPersisto agent, and restart it. From an administrator session, open **Settings → Administration → Applications** and verify the displayed issuer/discovery URL. Register exact application callbacks and save any generated confidential secret in the application server's secret store.
...
Dynamic client registration, implicit/hybrid and password grants, device authorization, SAML, general upstream identity federation beyond the pinned Google client, mandatory MFA, account recovery, general email replacement, DPoP, PAR/JAR, resource indicators, automated signing-key rotation, multi-tenant realms, high availability, and OpenID conformance certification remain outside this revision. These are explicit scope boundaries, not undocumented assumptions about Keycloak equivalence.
