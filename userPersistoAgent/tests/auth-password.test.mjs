...
test('passwordless methods stay ordinary account defaults and an empty or short override disables administrator sign-in', async () => {
    ...
    assert.equal((await wizardConfiguration()).adminPassword, true, 'fresh installations offer the default administrator password');
    ...
    process.env.USERPERSISTO_ADMIN_PASSWORD = 'tiny';
    admin.resetAdministratorPasswordForTests();
    assert.equal((await wizardConfiguration()).adminPassword, false);
    await assert.rejects(setup.claimAdministrator('tiny'), { code: 'admin_password_unavailable' });
...
test('guessing limits hold per trusted source and globally, before any account exists and across restarts', async () => {
    ...
    await assert.rejects(admin.verifyAdministratorPassword({ password: 'tiny', rateSource: SOURCE_B }), { code: 'authentication_failed' });
