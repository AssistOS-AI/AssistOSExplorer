...
`wizard-flows.mjs` drives the shared wizard on both renderers. An isolated fresh installation first proves the opening form orders **Continue with Google**, **Email**, **Admin password**, and **Sign in**, with no separate administrator screen. It submits the default `admin` password and an optional unverified contact email, checks that an incorrect password is cleared, and signs back in to the same administrator without an email. A separate email-first installation leaves the password empty and proves the first-run message, that requesting a code claims nothing, that a verified email signup claims the installation without activating the default administrator password, the Login/Register confirmation transitions, the method chooser with a wrong code followed by the right one, email-code sign-in resolving the existing installation administrator, and a Google denial returning to the live wizard. On the OIDC side the identity service runs on `localhost` and the application on `127.0.0.1`, a different site, and opens the authorization in a popup. It proves the `up_browser` proof is HttpOnly, SameSite=Strict and scoped to `/service/`, that the native `email-verify` POST carries it from the popup, that a wrong code re-renders the interaction with a visible error, and that administrator email-code sign-in still requires separate application consent. Screenshots are taken before any code or password is typed and are written to `.ploinky/test-artifacts/userpersisto-wizard/<run>/` (override with `WIZARD_BROWSER_ARTIFACT_DIR`).

The wizard runner explicitly unsets `PROD` for the combined form, then sets it to `false` while an email challenge is pending. It verifies that refreshing suppresses the saved email and challenge and displays only Google, completes controlled Google sign-in through both SSO and OIDC, preserves cancellation messages, and retains separate OIDC consent. It also checks that `PROD` set to an empty string still selects production and that unavailable Google exposes no local credential fallback. Both Google-only screens are captured alongside the nonproduction form.
