...
`wizard-flows.mjs` drives the shared wizard on both renderers. On the Router SSO page it proves the first-run message, that requesting a code claims nothing, that a verified email signup claims the installation, the Login/Register confirmation transitions, the method chooser with a wrong code followed by the right one, email-code sign-in resolving the existing installation administrator, and a Google denial returning to the live wizard. On the OIDC side the identity service runs on `localhost` and the application on `127.0.0.1`, a different site, and opens the authorization in a popup. It proves the `up_browser` proof is HttpOnly, SameSite=Strict and scoped to `/service/`, that the native `email-verify` POST carries it from the popup, that a wrong code re-renders the interaction with a visible error, and that administrator email-code sign-in still requires separate application consent. Screenshots are taken before any code is typed and are written to `.ploinky/test-artifacts/userpersisto-wizard/<run>/` (override with `WIZARD_BROWSER_ARTIFACT_DIR`).
