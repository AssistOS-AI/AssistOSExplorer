...
export const ADMIN_PASSWORD_MIN_LENGTH = 5;
...
async function configuredPassword(store) {
    ...
    if (defaultPassword) {
        ...
        if (setup.complete && !(setup.method === 'adminPassword' && state?.defaultPassword === true)) return { password: '', defaultPassword: false };
        value = DEFAULT_ADMIN_PASSWORD;
    }
    if (typeof value !== 'string' || !value) return { password: '', defaultPassword: false };
    if (value.length < ADMIN_PASSWORD_MIN_LENGTH || value.length > ADMIN_PASSWORD_MAX_LENGTH || !value.trim()) {
        ...
        return { password: '', defaultPassword: false };
    }
    return { password: value, defaultPassword };
}

export async function isAdministratorPasswordConfigured() {
    return (await configuredPassword(await getStore())).password !== '';
}
...
async function syncLocked() {
    ...
    return serializePersisted('users', async () => {
        ...
        if (next !== current || (!next && record)) {
            const setup = await readInstallationSetup(store);
            const ownedCredential = current && (current.defaultPassword !== true || setup.method === 'adminPassword');
...
export async function administratorPasswordUsableFor(userId) {
    if (!(await isAdministratorPasswordConfigured()) || typeof userId !== 'string' || !userId) return false;
...
export async function assertAdministratorPasswordProof(store, { userId, credentialVersion }) {
    ...
    return Boolean(await isAdministratorPasswordConfigured() && state && administrator && administrator.id === userId
...
export async function completeAdministratorPassword({ password, rateSource, contactEmail = '', validateParent }) {
    ...
    return serializePersisted('users', async () => {
        ...
        if (validateParent) await validateParent();
        const { state } = await readState(store);
        if (!(await isAdministratorPasswordConfigured()) || !state || administratorPasswordVersion(state) !== proof.credentialVersion) throw adminError('authentication_failed', 401);
        const setup = await readInstallationSetup(store);
        if (!setup.complete) {
